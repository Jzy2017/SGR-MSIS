...
class Transform(torch.nn.Module):
    ...
    def __init__(self, height, width, device,batch_size):
        super().__init__()
        self.grid = torch.reshape(torch.reshape(torch.unsqueeze(self._meshgrid(height, width),0), [-1]).repeat(batch_size),[batch_size, 3, -1])
        
        # base = _repeat(tf.range(num_batch) * dim1, out_height * out_width)
        self.base = self._repeat(torch.arange(batch_size) * (width * height), height * width)
        if device > -1:
